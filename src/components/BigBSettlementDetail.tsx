...
import { type BigBSettlement } from './BigBSettlementList';
import { toast } from 'sonner';

interface BigBSettlementDetailProps {
  settlement: BigBSettlement;
  onBack: () => void;
}

export function BigBSettlementDetail({ settlement, onBack }: BigBSettlementDetailProps) {
  ...
  // 模拟计算数据
  const platformPrice = settlement.orderAmount * 0.85; // 平台分销价
  const refundP1 = settlement.refundAmount * 0.85; // 退款对应P1部分
  const commission = settlement.orderAmount * 0.02; // 小B佣金
  const refundCommission = settlement.refundAmount * 0.02; // 退款对应佣金
  const bigbDiscount = settlement.orderAmount * 0.03; // 大B出资优惠
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-3 gap-6">
            <div>
              ...
              <p className="text-sm text-gray-900">{settlement.bigbName}</p>
            </div>
            <div>
              <Label className="text-gray-500 text-xs font-medium mb-1.5 block">业务模式</Label>
              <p className="text-sm text-gray-900">{getBusinessModelLabel(settlement.businessModel)}</p>
            </div>
            <div>
              <Label className="text-gray-500 text-xs font-medium mb-1.5 block">联系方式</Label>
              <p className="text-sm text-gray-900">138****8888</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-3 gap-6">
            ...
            <div>
              <Label className="text-gray-500 text-xs font-medium mb-1.5 block">订单金额</Label>
              ...
              <div className="bg-gray-50 rounded-lg p-4 space-y-2">
                {settlement.refundAmount > 0 && (
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-sm font-medium text-gray-900">
                      ¥{settlement.refundAmount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                )}
                <div className="flex justify-between items-center">
                  <span className="text-sm text-gray-600">
                    平台分销价（扣除退款对应部分 ¥{refundP1.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}）
                  </span>
                  <span className="text-sm font-medium text-gray-900">
                    ¥{(platformPrice - refundP1).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
                {commission > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">
                      小B佣金（扣除退款对应部分 ¥{refundCommission.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}）
                    </span>
                    <span className="text-sm font-medium text-gray-900">
                      ¥{(commission - refundCommission).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                )}
                {bigbDiscount > 0 && (
                  <div className="flex justify-between items-center">
                    <span className="text-sm text-gray-600">大B出资优惠</span>
                    <span className="text-sm font-medium text-gray-900">
                      ¥{bigbDiscount.toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </span>
                  </div>
                )}
                <div className="flex justify-between items-center pt-2 border-t">
                  <span className="text-sm font-medium text-gray-700">小计</span>
                  <span className="text-sm font-bold text-gray-900">
                    ¥{(settlement.refundAmount + (platformPrice - refundP1) + (commission - refundCommission) + bigbDiscount).toLocaleString('zh-CN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                  </span>
                </div>
              </div>
            </div>
