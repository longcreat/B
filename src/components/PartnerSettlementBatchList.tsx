...
import { 
  getMockPartnerSettlementBatches, 
  type PartnerSettlementBatch,
  type PartnerBatchStatus,
  type PartnerType,
} from '../data/mockSettlementBatches';

export { type PartnerSettlementBatch };

interface PartnerSettlementBatchListProps {
  onViewBatchDetail?: (batch: PartnerSettlementBatch) => void;
}

export function PartnerSettlementBatchList({ onViewBatchDetail }: PartnerSettlementBatchListProps) {
  ...
  const [approvingBatch, setApprovingBatch] = useState<PartnerSettlementBatch | null>(null);
  const [approveAction, setApproveAction] = useState<'approve' | 'reject'>('approve');
  const [approveComment, setApproveComment] = useState('');
  const [rejectReason, setRejectReason] = useState('');

  // 使用 mock 数据
  const allBatches = getMockPartnerSettlementBatches();
    ...
    return allBatches.filter((batch) => {
      ...
      if (searchQuery) {
        ...
        if (!batch.batchId.toLowerCase().includes(query) && 
            !batch.partnerName.toLowerCase().includes(query)) {
          return false;
        }
      }

      // 小B类型筛选
      if (filterPartnerType !== 'all' && batch.partnerType !== filterPartnerType) {
        return false;
      }
  ...
  }, [allBatches, searchQuery, filterPartnerType, filterStatus, filterPeriodStart, filterPeriodEnd, filterCreateDateStart, filterCreateDateEnd, filterAmountMin, filterAmountMax]);
  ...
  }, [searchQuery, filterPartnerType, filterStatus, filterPeriodStart, filterPeriodEnd, filterCreateDateStart, filterCreateDateEnd, filterAmountMin, filterAmountMax]);
  ...
  const handleBatchCredit = () => {
    ...
    toast.success(`已批量计入 ${selected.length} 条批次`);
    setSelectedBatches(new Set());
  };

  // 审批单个批次
  const handleApprove = (batch: PartnerSettlementBatch) => {
  ...
  // 计入账户
  const handleCredit = (batch: PartnerSettlementBatch) => {
      ...
      <div className="p-6 space-y-6">
        ...
        <Breadcrumb>
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbPage>客户结算</BreadcrumbPage>
          </BreadcrumbList>
        </Breadcrumb>
          ...
          <CardContent className="py-4">
            ...
            {showFilters && (
              <div className="mt-4 pt-4 border-t grid grid-cols-2 md:grid-cols-4 gap-4">
                <div>
                  <Label className="text-sm">小B类型</Label>
        ...
        <Card>
          <CardContent className="p-0">
            <style>{`
              .partner-batch-table-container table td:last-child,
              .partner-batch-table-container table th:last-child {
              ...
              .partner-batch-table-container table td:first-child,
              .partner-batch-table-container table th:first-child {
                position: sticky !important;
                left: 0 !important;
                background: white !important;
                z-index: 10 !important;
            ...
            `}</style>
            <div className="overflow-x-auto partner-batch-table-container">
              <table className="w-full border-collapse" style={{ minWidth: '1800px' }}>
                <thead>
                  <tr className="border-b">
                    ...
                    <th className="text-left p-3 font-medium text-sm bg-gray-50" style={{ minWidth: '150px' }}>小B名称</th>
                    <th className="text-left p-3 font-medium text-sm bg-gray-50" style={{ minWidth: '120px' }}>小B类型</th>
                    <th className="text-left p-3 font-medium text-sm bg-gray-50" style={{ minWidth: '200px' }}>结算周期</th>
                    ...
                    <th className="text-left p-3 font-medium text-sm bg-gray-50" style={{ minWidth: '150px' }}>计入时间</th>
                  ...
                  {paginatedBatches.length === 0 ? (
                    ...
                    paginatedBatches.map((batch) => (
                      <tr key={batch.batchId} className="border-b hover:bg-gray-50">
                        ...
                        <td className="p-3 text-sm">{batch.partnerName}</td>
                        <td className="p-3 text-sm">{getPartnerTypeLabel(batch.partnerType)}</td>
                        <td className="p-3 text-sm">{batch.settlementPeriodStart} 至 {batch.settlementPeriodEnd}</td>
                        ...
                        <td className="p-3 text-sm">{batch.creditedAt || '--'}</td>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4 p-4 bg-gray-50 rounded-lg">
                  ...
                  <div>
                    ...
                    <p className="mt-1">{approvingBatch.partnerName}</p>
                  </div>
                  <div>
                    <Label className="text-sm text-gray-600">结算周期</Label>
                    <p className="mt-1">{approvingBatch.settlementPeriodStart} 至 {approvingBatch.settlementPeriodEnd}</p>
                  </div>
                  <div>
