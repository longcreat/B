...
interface SupplierSettlementListProps {
  onViewDetail?: (settlement: SupplierSettlement) => void;
}

// Mock数据生成
const generateMockData = (): SupplierSettlement[] => {
  const data: SupplierSettlement[] = [];
  const supplierNames = ['XX酒店供应商', 'YY旅游供应商', 'ZZ酒店集团', 'AA国际酒店', 'BB连锁酒店'];
  const statuses: SupplierSettlement['settlementStatus'][] = ['pending', 'ready', 'settled', 'cancelled'];
  
  for (let i = 1; i <= 50; i++) {
    const orderCost = 800 + Math.random() * 1200;
    const refundCost = Math.random() > 0.7 ? Math.random() * 300 : 0;
    data.push({
      settlementId: `SETTLE-S-${String(i).padStart(6, '0')}`,
      orderId: `ORD-${String(i).padStart(6, '0')}`,
      supplierName: supplierNames[i % supplierNames.length],
      orderCost,
      refundCost,
      settlementAmount: orderCost - refundCost,
...
export function SupplierSettlementList({ onViewDetail }: SupplierSettlementListProps) {
    ...
    return allSettlements.filter((settlement) => {
      if (searchQuery) {
        ...
        if (!settlement.settlementId.toLowerCase().includes(query) &&
            ...
            !settlement.supplierName.toLowerCase().includes(query)) {
          return false;
        }
      }
  ...
  }, [allSettlements, searchQuery, filterStatus, filterAmountMin, filterAmountMax, filterTimeStart, filterTimeEnd]);
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, filterStatus, filterAmountMin, filterAmountMax, filterTimeStart, filterTimeEnd]);
          ...
          <CardContent className={showFilters ? "py-4" : "py-3"}>
            <div className="flex items-center gap-3">
              <div className="relative w-64">
                ...
                <Input
                  placeholder="搜索结算记录ID、订单号、供应商名称"
                  value={searchQuery}
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="border-b bg-gray-50">
                    ...
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">订单成本</th>
                    <th className="px-4 py-3 text-left text-sm font-medium text-gray-700">退款成本</th>
                  ...
                  {paginatedSettlements.length === 0 ? (
                    ...
                    paginatedSettlements.map((settlement) => (
                      <tr key={settlement.settlementId} className="border-b hover:bg-gray-50">
                        ...
                        <td className="px-4 py-3 text-sm text-gray-900">{settlement.supplierName}</td>
